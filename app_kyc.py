...
INDUSTRIES = ["Manufacturing","Oil & Gas Trading","Real Estate","Agriculture",
...
with st.sidebar:
    ...
    country      = st.selectbox("Country of Incorporation",
        ["Nigeria","Ghana","Kenya","South Africa","Other"])
 
    st.markdown("---")
    st.markdown("### Run Mode")
    run_mode = st.radio("Select mode:",
    ...
    run_btn = st.button("Run KYC Screening", type="primary",
    ...
    if "kyc_result" in st.session_state:
        ...
        st.markdown("---")
        st.caption(f"Searches run: {len(meta.get('queries_run',[]))}")
        ...
        with st.expander(f"Director / Officer {did}", expanded=True):
            ...
            with c1:
                d_name     = st.text_input(f"Full Name #{did}",
                    placeholder="e.g. Chidi Okonkwo", key=f"name_{did}")
                d_position = st.text_input(f"Position #{did}",
                    placeholder="e.g. Managing Director", key=f"pos_{did}")
                d_nat      = st.selectbox(f"Nationality #{did}",
                    ["Nigerian","Ghanaian","Kenyan","British","American","Other"],
                    key=f"nat_{did}")
            with c2:
                d_pep_self = st.checkbox(f"Currently holds / held public office #{did}",
                    key=f"pep_{did}")
                d_pep_fam  = st.checkbox(f"Family member holds public office #{did}",
                    key=f"pepfam_{did}")
            ...
            if len(st.session_state.directors)>1:
                if st.button(f"Remove #{did}", key=f"rm_{did}"):
                    st.session_state.directors = [x for x in st.session_state.directors if x["id"]!=did]
                    st.rerun()
            directors_data.append({
                "name": d_name, "position": d_position,
                "nationality": d_nat,
...
with tab2:
    for d in r.get("director_screenings",[]):
        ...
        with st.expander(
            ...
            pc  = "red" if d.get("pep_status") else "green"
            c1.metric("PEP Status", pep)
            s = "HIT" if d.get("sanctions_match") else "Clear"
            sc2 = "red" if d.get("sanctions_match") else "green"
            c2.metric("Sanctions", s)
            o = "HIT" if d.get("ofac_match") else "Clear"
            oc = "red" if d.get("ofac_match") else "green"
            c3.metric("OFAC",o)
            am = d.get("adverse_media","NONE")
            amc = "green" if am=="NONE" else "orange" if am=="MINOR" else "red"
...
with tab4:
    ...
    st.text_area("Override:", value=r.get("credit_memo_narrative",""),
...
if st.button("Start New Screening"):
    for k in ["kyc_result","_directors"]:
