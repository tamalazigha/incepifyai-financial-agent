...
st.set_page_config(
    page_title="IncepifyAI — Collateral & Security",
    page_icon="🛡️",
    layout="wide",
)
...
st.markdown(
    ...
    <span style='font-size:13px;color:grey;margin-left:12px'>
        Collateral & Security Agent · Agent 2 of 8</span>
    </div>""",
...
# ── Session state: collateral items ───────────────────────────
        ...
        with st.expander(f"Collateral Item {item_id}", expanded=True):
            ...
            with col1:
                ...
                c_omv  = st.number_input(f"Open Market Value (NGN '000) #{item_id}",
                    min_value=0, value=500000, step=10000, key=f"omv_{item_id}")
            ...
            with col2:
                ...
                d_title = st.checkbox("Title document available",
                    key=f"title_{item_id}", value=True)
                d_val   = st.checkbox("Valuation report available",
                    key=f"val_{item_id}", value=True)
                d_ins   = st.checkbox("Insurance in place (bank noted)",
                    key=f"ins_{item_id}")
                d_cac   = st.checkbox("CAC charge registered",
                    key=f"cac_{item_id}")
                d_gov   = st.checkbox("Governor's Consent obtained",
                    key=f"gov_{item_id}")
                d_enc   = st.checkbox("Confirmed encumbrance-free",
                    key=f"enc_{item_id}", value=True)
...
s_colors = {"ADEQUATE": "green", "SHORTFALL": "red", "BORDERLINE": "orange"}
sc = s_colors.get(status, "grey")

st.markdown(f"## Security Assessment: :{sc}[**{status}**]")
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "Coverage Summary", "Item Analysis", "Risk Flags", "Memo Draft", "Raw JSON"
])
...
with tab1:
    ...
    c2.metric("Shortfall / Surplus", f"NGN {abs(surplus):,.0f}k",
...
with tab2:
    for item in r.get("collateral_analysis", []):
        with st.expander(
            f"Item {item['item_number']}: {item.get('description', '—')} — {item.get('quality_tier', '—')}",
            expanded=True,
        ):
            c1, c2, c3 = st.columns(3)
            c1.metric("Open Market Value",
                      f"NGN {item.get('open_market_value_000', 0):,.0f}k")
            c2.metric("Advance Rate", f"{item.get('advance_rate_pct', 0):.0f}%")
            c3.metric("Adjusted Value",
                      f"NGN {item.get('adjusted_value_000', 0):,.0f}k")
            enf = item.get("enforcement_risk", "—")
            ec  = "red" if enf == "HIGH" else "orange" if enf == "MEDIUM" else "green"
            st.markdown(f"**Enforcement Risk:** :{ec}[{enf}]")
...
if st.button("Start New Analysis"):
    for k in ["coll_result", "_items_data", "collateral_result"]:
