...
st.set_page_config(
    page_title="IncepifyAI — Financial Analysis",
    page_icon="📊",
    layout="wide",
)
...
st.markdown(
    ...
    <span style='font-size:13px;color:grey;margin-left:12px'>
        Financial Analysis Agent · Agent 1 of 8</span>
    </div>""",
...
if run_btn:
    if use_mock:
        ...
        st.session_state["company"] = company_name or "Test Manufacturing Co Ltd"
    else:
    ...
    # ── Shared suite keys ─────────────────────────────────────
    st.session_state["financial_result"] = st.session_state["result"]
    st.session_state["applicant_name"]   = st.session_state.get("company", "")
...
if "result" not in st.session_state:
    ...
    st.info(
        "Upload a financial statement PDF in the sidebar, "
        "or tick 'Use mock data' to test the full interface."
    )
    st.stop()
...
# ── Tab 1: Key Ratios ─────────────────────────────────────────
with tab1:
    # The JSON structure from financial_agent.py uses "key_ratios"
    # with nested sub-dicts: profitability, liquidity, leverage, cashflow
    ratios = result.get("key_ratios", {})
 
    if not ratios:
        st.info("No ratio data found. Check the Raw JSON tab to see what keys were returned.")
        st.json({k: v for k, v in result.items() if k != "_metadata"})
    else:
        c1, c2 = st.columns(2)
        with c1:
            ...
            for k, v in ratios.get("liquidity", {}).items():
                if v is not None:
                    ...
                    delta_color = "normal" if v >= 1.0 else "inverse"
                    st.metric(label, f"{v:.2f}x", delta_color=delta_color)
        ...
        with c2:
            ...
            for k, v in ratios.get("leverage", {}).items():
                if v is not None:
                    label = k.replace("_", " ").title()
                    st.metric(label, f"{v:.2f}x")
 
            st.markdown("#### Cashflow")
            for k, v in ratios.get("cashflow", {}).items():
                if v is not None:
                    label = k.replace("_000", " (NGN 000)").replace("_", " ").title()
                    val   = f"{v:,.0f}" if "_000" in k else f"{v:.2f}"
                    st.metric(label, val)
    ...
    if trend:
        ...
        if trend.get("revenue_3yr_cagr_pct") is not None:
            c1.metric("Revenue 3yr CAGR", f"{trend['revenue_3yr_cagr_pct']:.1f}%")
        c2.metric("Revenue Trend",       trend.get("revenue_trend", "—"))
        c3.metric("Margin Trend",        trend.get("margin_trend",  "—"))
...
with tab3:
    ...
    st.caption(
        f"DSCR minimum: 1.20x  ·  "
        f"Downside: {down.get('revenue_stress_pct', 0):.0f}% revenue stress"
    )
    if base.get("narrative"):
        st.markdown(f"**Base case:** {base['narrative']}")
    if down.get("narrative"):
        st.markdown(f"**Downside:** {down['narrative']}")
...
with tab4:
    ...
    with c1:
        ...
        for s in hl.get("key_strengths", []):
            st.markdown(f"- {s}")
    with c2:
        st.markdown("**Key Concerns**")
        for c in hl.get("key_concerns", []):
            st.markdown(f"- {c}")
    st.markdown("---")
    st.markdown("**Section 13 — Financial Analysis (edit before submitting):**")
    st.text_area(
        "",
        value=rec.get("narrative_summary", ""),
        height=180,
...
if st.button("Start New Analysis"):
    for k in ["result", "company", "financial_result", "applicant_name"]:
