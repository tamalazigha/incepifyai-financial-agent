...
st.set_page_config(page_title="IncepifyAI — Industry Intelligence Agent",
                   page_icon="telescope", layout="wide")
...
CBN_SECTORS = [
    ...
    "Healthcare & Pharmaceuticals","ICT & Technology",
    "Manufacturing","Mining & Quarrying","Oil & Gas",
    "Power & Energy","Real Estate","Retail & Trade",
...
with st.sidebar:
    ...
    sub_sector   = st.text_input("Sub-Sector (optional)",
        placeholder="e.g. Food & Beverages · Cement · Textiles")
    country      = st.selectbox("Country",
        ["Nigeria","Ghana","Kenya","South Africa","Other"])
    company_desc = st.text_area("Brief Company Description (optional)",
        placeholder="e.g. Produces packaged food products; 8-year client; estimated 2% market share",
        height=100,
        help="More context = more targeted searches = better output")
    use_mock  = st.checkbox("Use mock data (no API cost)")
    st.markdown("---")
    run_btn = st.button("Run Industry Analysis", type="primary",
...
if "ind_result" not in st.session_state:
    ...
    c1.metric("Searches per run","3–6","web queries auto-selected")
    c2.metric("Sources","8 categories","CBN, NBS, Reuters, NGX...")
    c3.metric("Cost per run","~$0.05","Sonnet + web search tokens")
    st.info("Select the sector and enter the company name in the sidebar, then click Run.")
    st.stop()
...
st.markdown(f"## :{oc}[{outlook} Outlook]  ·  :{rc}[{rating} Risk]  ·  Credit Implication: **{impl}**")
st.markdown(f"> {r.get('credit_memo_narrative','')}")
st.markdown("---")
 
tab1,tab2,tab3,tab4,tab5 = st.tabs([
    "Market Overview","Competitive Landscape","Trends & Risks",
    "Memo Draft","Raw JSON"])
 
with tab1:
    md = r.get("market_data",{})
    c1,c2,c3,c4 = st.columns(4)
    c1.metric("Market Size (USD)", f"${md.get('market_size_usd_bn','N/A')}bn"
              if md.get("market_size_usd_bn") else "N/A")
    c2.metric("Market Size (NGN)", f"₦{md.get('market_size_ngn_bn','N/A')}bn"
              if md.get("market_size_ngn_bn") else "N/A")
    c3.metric("Historical CAGR (5yr)", f"{md.get('cagr_historical_5yr_pct','N/A')}%"
              if md.get("cagr_historical_5yr_pct") is not None else "N/A")
    c4.metric("Projected CAGR (5yr)", f"{md.get('cagr_projected_5yr_pct','N/A')}%"
    ...
    st.markdown(f"**Sector:** {ip.get('sector','—')} · **Sub-sector:** {ip.get('sub_sector','—')} · **CBN Classification:** {ip.get('cbn_sector_classification','—')}")
    st.markdown(f"**Outlook commentary:** {ia.get('outlook_commentary','')}")
...
with tab2:
    ...
    if op.get("key_differentiators"):
        ...
        for d in op["key_differentiators"]: st.markdown(f"- {d}")
    if op.get("key_vulnerabilities"):
        st.markdown("**Vulnerabilities:**")
        for v in op["key_vulnerabilities"]: st.markdown(f"- {v}")
